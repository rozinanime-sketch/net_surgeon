...
pub async fn run_socks5_udp_processor(socks5_udp_port: &str, log_tx: LogSender) {
    ...
    log(&log_tx, LogLevel::Success, format!("SOCKS5 UDP сервер слушает на {}", socks5_udp_port));
    ...
    loop {
        match server_socket.recv_from(&mut buffer).await {
            Ok((bytes_read, client_src_addr)) => {
                ...
                tokio::spawn(async move {
                    ...
                    if !table.contains_key(&client_src_addr) {
                        log(&log_tx, LogLevel::Warning, format!("SOCKS5 QUIC JUNK: новый поток к {}, обфускация", dst_addr));
                        ...
                        log(&log_tx, LogLevel::Success, format!("SOCKS5 Junk пакеты ({}) отправлены -> {}", jc, dst_addr));
                    } else {
                        drop(table);
                    }
            ...
            Err(e) => {
                log(&log_tx, LogLevel::Error, format!("Ошибка SOCKS5 UDP: {}", e));
            }
        }
    }
}
