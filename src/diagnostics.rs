...
pub enum ProbeOutcome {
    /// TCP-соединение не открылось вовсе (таймаут/refused)
    ConnectFailed,
    /// TCP открылся, но запись ClientHello сразу упала (RST на первый же пакет)
    ResetOnWrite,
    /// TCP+запись OK, чтение таймаутит — пакеты молча дропаются, RST не шлют
    SilentDrop,
    /// TCP+запись OK, чтение вернуло ошибку сброса соединения — DPI среагировал на содержимое
    ResetAfterHello,
    /// Получен валидный ответ — соединение прошло
    Success,
}
...
impl ProbeOutcome {
    pub fn description(&self) -> &'static str {
        match self {
            ProbeOutcome::ConnectFailed => "TCP не открылся — блокировка по IP/порту",
            ProbeOutcome::ResetOnWrite => "Соединение сброшено сразу при отправке — DPI реагирует мгновенно",
            ProbeOutcome::SilentDrop => "Пакеты молча дропаются, ответа нет — silent drop DPI",
            ProbeOutcome::ResetAfterHello => "Соединение сброшено после ClientHello — DPI анализирует SNI и режет по содержимому",
            ProbeOutcome::Success => "TLS-рукопожатие прошло успешно",
        }
    }
}
