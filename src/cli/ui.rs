...
fn draw_diagnostics(frame: &mut Frame, area: Rect, screen: &super::app::DiagnosticsScreen) {
    ...
    if let Some(buf) = &screen.input_buffer {
        let line = Line::from(vec![
            Span::styled("Домен: ", Style::default().fg(Color::DarkGray)),
                              Span::styled(format!("{}█", buf), Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        ]);
        let hint = Line::from(Span::styled(
            "Enter — запустить, Esc — отмена",
            Style::default().fg(Color::DarkGray),
        ));
        let p = Paragraph::new(vec![line, Line::from(""), hint]);
        frame.render_widget(p, inner);
    } else if screen.running {
        let p = Paragraph::new("Тестирую… результат появится в логах")
        .style(Style::default().fg(Color::Yellow));
        frame.render_widget(p, inner);
    } else {
        let p = Paragraph::new(vec![
            Line::from("Результат смотрите в панели логов."),
                               Line::from(""),
                               Line::from(Span::styled("n — новый тест    Esc/q — назад", Style::default().fg(Color::DarkGray))),
        ]);
        frame.render_widget(p, inner);
    }
}
...
fn draw_menu(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let list = List::new(items)
    .block(Block::default().borders(Borders::ALL).title(" Меню ").border_style(border_style));
...
fn draw_logs(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let title = if app.log_autoscroll {
        ...
        " Логи (прокрутка, End чтобы вернуться) ".to_string()
    };
