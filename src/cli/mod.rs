            ...
            match events::handle_event(app, event) {
                ...
                events::Action::RunDiagnostics(domain) => {
                    ...
                    rt.spawn(async move {
                        ...
                        let direct_verdict = if result.direct_tls_ok {
                            "TLS OK — DPI не блокирует"
                        } else if result.direct_connect_ok {
                            "TCP OK, TLS не прошёл — DPI режет по SNI"
                        } else {
                            "TCP не прошёл — заблокировано на уровне IP/порта"
                        };

                        let bypass_verdict = if result.bypass_tls_ok {
                            "TLS OK — обход работает"
                        } else if result.bypass_connect_ok {
                            "TCP OK, TLS не прошёл — обход не помог"
                        } else {
                            "TCP не прошёл — заблокировано на уровне IP/порта"
                        };

                        log(&log_tx, LogLevel::Info, format!("Диагностика {}: напрямую — {}", domain, direct_verdict));
                        log(&log_tx, LogLevel::Info, format!("Диагностика {}: с обходом — {}", domain, bypass_verdict));

                        if !result.direct_tls_ok && result.bypass_tls_ok {
                            log(&log_tx, LogLevel::Success, format!("{}: обход реально помогает!", domain));
                        } else if result.direct_tls_ok {
                            log(&log_tx, LogLevel::Info, format!("{}: блокировки не обнаружено", domain));
                        } else if !result.bypass_tls_ok {
