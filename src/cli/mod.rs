            ...
            match events::handle_event(app, event) {
                ...
                events::Action::RunDiagnostics(domain) => {
                    ...
                    rt.spawn(async move {
                        ...
                        log_nested_t(&log_tx, LogLevel::Info, "log.diag_bypass", "verdict", result.bypass.description_key(), vec![
                            ("domain", domain.clone()),
                        ]);
                ...
                events::Action::StartProxy => {
                    if !app.proxy_started {
                        ...
                        app.push_log_t(LogLevel::Warning, "startup.already_running", vec![]);
                    }
                }
                events::Action::None => {}
            }
