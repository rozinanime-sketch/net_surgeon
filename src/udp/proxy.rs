use std::sync::Arc;
use std::time::Duration;
use tokio::net::UdpSocket;

use crate::config::Ranges;
use crate::bypass::fragment;
use crate::cli::{LogSender, log_t, LogLevel};
use crate::metrics::Metrics;

pub async fn run_udp_proxy(listen_address: String, udp_target: String, ranges: Ranges, log_tx: LogSender, metrics: Arc<Metrics>) {
    let socket = UdpSocket::bind(&listen_address).await
    .expect("Ошибка: не удалось занять UDP порт");
    let socket = Arc::new(socket);

    log_t(&log_tx, LogLevel::Success, "log.udp_listening", vec![("addr", listen_address.clone())]);

    {
        let target = udp_target.clone();
        let metrics = Arc::clone(&metrics);
        let log_tx = log_tx.clone();
        tokio::spawn(async move {
            loop {
                let ok = dns_health_check(&target).await;
                metrics.set_dns_ok(ok);
                if !ok {
                    log_t(&log_tx, LogLevel::Warning, "log.dns_health_failed", vec![("target", target.clone())]);
                }
                tokio::time::sleep(Duration::from_secs(15)).await;
            }
        });
    }

    let mut buffer = [0u8; 65535];
    loop {
        match socket.recv_from(&mut buffer).await {
            Ok((len, client_addr)) => {
                let data = buffer[..len].to_vec();
                let main_socket = Arc::clone(&socket);
                let target = udp_target.clone();
                let ranges_clone = ranges.clone();
                let log_tx = log_tx.clone();
                let metrics = Arc::clone(&metrics);

                tokio::spawn(async move {
                    let upstream_socket = match UdpSocket::bind("0.0.0.0:0").await {
                        Ok(s) => s,
                             Err(_) => return,
                    };

                    metrics.add_rx(data.len() as u64);

                    if upstream_socket.send_to(&data, &target).await.is_ok() {
                        let mut resp_buf = [0u8; 65535];
                        if let Ok(Ok((resp_len, _))) = tokio::time::timeout(
                            Duration::from_secs(4),
                                                                            upstream_socket.recv_from(&mut resp_buf),
                        ).await {
                            fragment::apply_udp_jitter(&ranges_clone).await;
                            let _ = main_socket.send_to(&resp_buf[..resp_len], client_addr).await;
                            metrics.add_tx(resp_len as u64);
                            log_t(&log_tx, LogLevel::Info, "log.udp_response", vec![
                                ("bytes", resp_len.to_string()),
                                  ("addr", client_addr.to_string()),
                            ]);
                        }
                    }
                });
            }
            Err(e) => log_t(&log_tx, LogLevel::Error, "log.udp_error", vec![("error", e.to_string())]),
        }
    }
}

async fn dns_health_check(target: &str) -> bool {
    let probe: [u8; 28] = [
        0x12, 0x34,
        0x01, 0x00,
        0x00, 0x01,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x06, b'g', b'o', b'o', b'g', b'l', b'e',
        0x03, b'c', b'o', b'm',
        0x00,
        0x00, 0x01,
        0x00, 0x01,
    ];

    // 1. Создаем сокет
    let socket = match UdpSocket::bind("0.0.0.0:0").await {
        Ok(s) => s,
        Err(_) => return false,
    };

    // 2. ВАЖНО: Делаем явный connect к целевому DNS-серверу,
    // чтобы ОС направляла ответы именно в этот сокет
    if socket.connect(target).await.is_err() {
        return false;
    }

    // 3. Отправляем пакет через обычный send (так как мы уже привязаны через connect)
    if socket.send(&probe).await.is_err() {
        return false;
    }

    // 4. Ждем ответ именно от этого сервера через recv
    let mut buf = [0u8; 512];
    matches!(
        tokio::time::timeout(Duration::from_secs(2), socket.recv(&mut buf)).await,
             Ok(Ok(_))
    )
}
