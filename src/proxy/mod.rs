...
pub async fn run_all(
    ...
    let udp_task = {
        ...
        let metrics = Arc::clone(&metrics);
        tokio::spawn(async move {
            udp::proxy::run_udp_proxy(udp_address, config.udp_target.clone(), config.ranges.clone(), log_tx, metrics).await;
        })
    };
    ...
    let quic_task = {
        ...
        let metrics = Arc::clone(&metrics);
        tokio::spawn(async move {
            udp::quic::run_udp_quic_proxy(config.quic.listen_port, config.quic.target.clone(), log_tx, metrics).await;
        })
    };
